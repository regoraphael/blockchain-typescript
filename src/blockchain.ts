import { generateHash, isHashProofed } from './helpers';
...
export default class Blockchain {
  ...
  private get genesisBlock():Block {
    ...
    const header: Block['header'] = {
      nonce: 0,
      blockHash: generateHash(JSON.stringify(payload)),
    };
  ...
  private generateValidBlock(payload: Block['payload']): Block {
    ...
    while (true) {
      const blockHash = generateHash(JSON.stringify(payload));
      const proofingHash = generateHash(blockHash + nonce);
      if (isHashProofed({
        hash: proofingHash,
  ...
  validateBlock(block: Block): Boolean {
    ...
    if (!isHashProofed({
      hash: generateHash(generateHash(JSON.stringify(payload)) + nonce),
      difficulty: this.difficult,
      prefix: this.powPrefix,
    })) {
      console.error(`Invalid block #${payload.sequence}`);
